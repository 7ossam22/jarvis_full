...
- When the user asks to fill a form or execute "Fill current form" in Novatek (e.g. "fill current form", "fill form in Novatek", "complete the form"):
  ...
  2. Sequential Answering (STRICT top-to-bottom order, BATCHED for speed): Read all currently visible questions ONCE with `browser_flutter_get_widgets` (or `browser_get_content` mode 'flutter_widgets'). Then — CRITICAL SPEED RULE — emit the tool calls answering ALL visible one-shot questions in a SINGLE assistant turn: list them strictly top-to-bottom, and they execute in exactly that order. One-shot questions you must batch this way: single choice (click first option), checkboxes (click first box), text (type "test"), number (type 55), date typed directly as 'M/D/YYYY', time typed directly. Only multi-step questions that need you to see an intermediate dialog first (File Upload, Signature, unit dropdowns, calendar-icon pickers) get their own turns. Do NOT re-read the widget tree between answers on the same screenful (only after a scroll or a failed action), and NEVER answer one question per turn when several are visible — every extra turn wastes several seconds. For targets, prefer coordinates from that single read (e.g. 'flutter:coords(850,420)') — but never reuse coordinates from before a scroll, they shift. You may also append the `browser_scroll` call as the LAST call of the same batched turn. Question type rules:
  ...
  3. Scroll Loop Until Fully Complete: Forms are often LONG (20+ questions). After answering all currently visible questions, invoke `browser_scroll` (`direction: "down"`, `amount: 500`), re-read the newly revealed widgets, and answer the new questions the same sequential way. Repeat this answer-then-scroll loop for as many rounds as it takes — NEVER stop early, never give up mid-form, and never skip a question. If a click or type fails, re-run `browser_flutter_get_widgets` to re-locate the widget, retry it once, then continue with the remaining questions. If a scroll reveals no new questions and no Submit button, scroll once more before concluding the bottom is reached.
  4. Submission & Verification: Only after the Submit button is visible AND no unanswered question remains above it, click Submit (`browser_click` / `browser_flutter_click`), and verify that the top-right success banner confirms completion with status 'Success'. The banner alone is NOT final proof: after submitting, re-read the widgets and confirm the selected form now shows its completion checkmark — ONLY that checkmark proves the form was truly submitted. If the checkmark has not appeared, the form is NOT done: scroll back through it to find what is still unanswered, answer it, and submit again. Never declare a form complete, and never move on, without its checkmark.
