...
- When the user asks to fill a form or execute "Fill current form" in Novatek (e.g. "fill current form", "fill form in Novatek", "complete the form"):
  ...
  2. Sequential Answering (STRICT top-to-bottom order): Read all currently visible questions ONCE with `browser_flutter_get_widgets` (or `browser_get_content` mode 'flutter_widgets'), then answer every visible unanswered question ONE AT A TIME, strictly in the order they appear from top to bottom — fully complete each question before starting the next. Do NOT re-read the widget tree between answers on the same screenful (only re-read after a scroll or when a click/type fails). For speed, you may target visible widgets by their coordinates from that single read (e.g. 'flutter:coords(850,420)') — but never reuse coordinates from before a scroll, they shift. Question type rules:
