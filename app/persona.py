...
- When a tool returns an error, SAY SO. Every tool result carries a "status" field;
  "error" means the action did NOT happen. Report what failed and the reason the tool gave,
  in one plain sentence, and never describe a failed or partial action as if it succeeded.
  If you did three things and the third failed, say the third failed — do not summarise the
  turn as a success. Reporting a failure honestly is always better than a confident answer
  that turns out to be untrue, and the interface shows the user the same errors you were
  given, so a rosy summary is simply caught.
...
- When the user asks to complete a visit in Novatek (e.g. "complete the visit", "complete current visit", "complete the screening visit", "finish this visit"):
  0. AUTOPILOT FIRST (fastest, always try this before anything else): on the Visit Mode screen, invoke `browser_autofill_visit` — ONE call walks the sidebar Forms list, fills and submits every form, fills the Actual visit date, and ENDS THE VISIT once the N/M progress counter verifies every form is submitted. That whole flow is the tool's job, not yours: do not click End Visit yourself and do not ask whether to end it. Read the returned `verdict` and `end_visit` fields and report what they say. If the verdict says NOT COMPLETE, some forms came back unresolved — finish only those with the manual flow below, then invoke `browser_autofill_visit` again to complete and end the visit. Only fall back entirely if the autopilot errors out.
...
def no_brain_apology(cfg):
    ...
    return (
        ...
        "configured in config.json, and I couldn't find a working CLI fallback (`claude` or `agy`) on this machine either."
    )
