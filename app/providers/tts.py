...
def get_tts_providers(cfg):
    ...
    tried: the one `voice.tts_provider` ("elevenlabs" | "fish_audio") names
    first, then any other backend with a real key as failover — a provider
    outage (or an unpaid account) degrades to the next real voice, not
    straight to the browser's robot voice. With no explicit choice the order
    is ElevenLabs first, for backward compatibility."""
